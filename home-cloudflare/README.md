...
3. Depois do deploy: Custom domains → adicione `alna.sale` (e `www.alna.sale`).
4. Aponte `store.alna.sale` para o app do Lovable (Lovable → Settings → Domains) com o
   registro CNAME que ele pedir, no DNS do Cloudflare.
5. **Só depois** que `store.alna.sale` estiver abrindo, troque o domínio raiz para o Pages.
